/* Build-time reader: checks the real filesystem at static-export time, then
   defers the decision to the pure resolver. Not unit-tested (it's the I/O
   boundary). Import only from Server Components. */
...
export function getAssetPresence(): AssetPresence {
  ...
  return resolveAssetPresence({
    ...
    cvAiExists: fs.existsSync(path.join(pub, 'cv-ai.pdf')),
